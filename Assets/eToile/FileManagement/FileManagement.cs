...
public static class FileManagement
{
    ...
    /// <summary>Load a WAV audio file into AudioClip</summary>
    public static AudioClip ImportAudio(string file, bool enc = false, bool checkSA = true, bool fullPath = false)
    {
        ...
        if (OpenWavParser.IsCompatible(audioFile))
        {
            AudioClip clip = OpenWavParser.ByteArrayToAudioClip(audioFile, GetFileNameWithoutExtension(file));
            return clip;
        }
        else
        {
    ...
    public static void SaveAudio(string name, AudioClip clip, bool enc = false, bool fullPath = false)
    {
        if (!name.ToLower().EndsWith(".wav"))
            name += ".wav";
        SaveRawFile(name, OpenWavParser.AudioClipToByteArray(clip), enc, fullPath);
    }
